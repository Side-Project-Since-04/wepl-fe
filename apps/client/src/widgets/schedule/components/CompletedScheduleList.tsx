...
const CompletedSchedulesList: React.FC<ScheduleCardProps> = ({ schedules }) => {
  if (schedules.length === 0) {
    return (
      <Card className="w-full h-[246px] flex flex-col items-center justify-center gap-4 border-none shadow-none">
        ...
        <TextBody1 className="text-gray-400">완료된 일정이 없습니다</TextBody1>
      </Card>
    );
  }
...
export default CompletedSchedulesList;
    ...
    <Accordion className="w-full" collapsible type="single">
      {schedules.map((schedule) => (
        <AccordionItem key={schedule.smallCategoryPk} value={`item-${schedule.smallCategoryPk}`}>
          <AccordionTrigger className="h-92" headerClassName="h-92 px-20 border-2">
            <div>
              <div className="flex gap-12 mb-6">
                <WeplBadge className="bg-gray-100 text-gray-600 rounded-lg w-40 h-22">완료</WeplBadge>
          ...
          <AccordionContent className="border-x-2 border-b-2 bg-gray-50">
